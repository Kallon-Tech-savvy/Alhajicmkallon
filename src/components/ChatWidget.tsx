...
import { MAX_MESSAGE_LENGTH, CLIENT_HISTORY_LIMIT, QUICK_QUESTIONS } from '@/lib/Chatconstant';
...
export function ChatWidget() {
  ...
  // Bridge for other components (e.g. the onboarding guide) to open Cairo
  // and optionally push in a question. A ref holds the latest sendMessage
  // so this listener — registered once on mount — never calls a stale
  // closure over `input`/`loading`/`messages`.
  ...
  useEffect(() => {
    function handleAsk(e: Event) {
      ...
      setOpen(true);
      if (question) {
        // Let the panel mount before pushing a message into it.
        setTimeout(() => sendMessageRef.current(question), 50);
      }
    }
    window.addEventListener('cairo:ask', handleAsk);
    return () => window.removeEventListener('cairo:ask', handleAsk);
  }, []);
  ...
  // Keep the ref current every render so the cairo:ask listener (registered
  // once, above) always calls a fresh closure with up-to-date state.
            ...
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] px-4 py-6 text-center">
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      ...
                      className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-[11px] leading-tight text-ivory/90 transition-colors duration-200 hover:border-[var(--color-pride-blue)]/40"
                    >
                      {q}
                    </button>
                  ))}
                ...
                <p className="mt-3 text-[11px] leading-relaxed text-muted">
                  For a strong inquiry, include your goal, audience, timing, and the outcome you expect. The assistant can also help you prepare the contact form content.
                </p>
              </div>
            )}
            {messages.map((m, i) => (
